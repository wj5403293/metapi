...
describe('final transformer hard-cut architecture', () => {
  ...
  it('forbids transformer imports from routes, oauth, token router, runtime executor, and fastify', () => {
    ...
    for (const file of files) {
      ...
      expect(source).not.toMatch(/from\s+['"][^'"]*routes\/proxy\//m);
      expect(source).not.toMatch(/from\s+['"][^'"]*services\/oauth\//m);
      expect(source).not.toMatch(/from\s+['"][^'"]*services\/tokenRouter\.js['"]/m);
      expect(source).not.toMatch(/from\s+['"][^'"]*routes\/proxy\/runtimeExecutor\.js['"]/m);
